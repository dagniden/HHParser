...
def test_get_user_input_str(monkeypatch):
    monkeypatch.setattr("builtins.input", lambda _: "  Python  ")
    result = CLI._CLI__get_user_input("Введите строку: ", str)
    assert result == "Python"


def test_get_user_input_int(monkeypatch):
    inputs = iter(["abc", "42"])
    monkeypatch.setattr("builtins.input", lambda _: next(inputs))
    printed = []
    monkeypatch.setattr("builtins.print", lambda msg: printed.append(msg))
    result = CLI._CLI__get_user_input("Введите число: ", int)
    assert result == 42
...
def test_get_user_choice_valid(monkeypatch):
    ...
    result = CLI._CLI__get_user_choice("Ваш выбор: ", ["да", "нет"])
    assert result == "да"
...
def test_ask_top_n_no(monkeypatch):
    monkeypatch.setattr("builtins.input", lambda _: "нет")
    result = CLI.ask_top_n()
    assert result is None


def test_ask_filter_range_yes(monkeypatch):
...
def test_ask_filter_range_no(monkeypatch):
    monkeypatch.setattr("builtins.input", lambda _: "нет")
    result = CLI.ask_filter_range()
    assert result == (None, None)


def test_ask_region_name_found(monkeypatch):
...
def test_ask_region_name_not_found(monkeypatch):
...
def test_ask_filter_by_word_yes(monkeypatch):
...
def test_ask_filter_by_word_no(monkeypatch):
    monkeypatch.setattr("builtins.input", lambda _: "нет")
    result = CLI.ask_filter_by_word()
    assert result is None
...
def test_display_vacancies(monkeypatch):
    vacancies = [
